...
describe('Get the current time and weather by location or postal codes', () => {
	...
	it('should return an error if location name is invalid', async () => {
		...
		expect(response).to.be.an('object')
		expect(response).to.have.property('message').to.equal('error')
		expect(response).to.have.property('reason').to.equal('Current time and weather of Location Could not be found, make sure you inputed a valid location')
	...
	it('should return an error if postal code is invalid', async () => {
		...
		expect(response).to.be.an('object')
		expect(response).to.have.property('message').to.equal('error')
		expect(response).to.have.property('reason').to.equal('Current time and weather of Postal Code Could not be found, make sure you inputed a valid postal code')
		expect(response).to.have.property('status').to.equal(404)
		} catch(error){
			return error
		}
